...
validation = True
model_name = "t5"
pretrainedmodel_path = "../LLMs/{}/".format(model)  # the path of the pre-trained model
model_name2 = "{}_{}_400_2000_{}epoch_{}batch".format(
    model, str(train_percentage), str(num_epochs), str(batch_size)
)
...
for project in project_list:
    ...
    def evaluate(prompt_model, dataloader, mode="test", diff_out_path=""):
        ...
        if mode =="test":
            ...
            with open(diff_out_path + "prediction.csv", "w", encoding="utf-8") as f:
                f = csv.writer(f)
                f.writerows(res_out)
            with open(diff_out_path + "diff.csv", "w", encoding="utf-8") as f:
                f = csv.writer(f)
                f.writerows(csv_out)
            with open(diff_out_path + "result.txt", "w", encoding="utf-8") as f:
                f.write(str(score))
        # shown one example
    ...
    output_dir = "../fine_tuned_model/{}/{}/{}/".format(model, project,train_percentage)
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    des=project+str(train_percentage)
    progress_bar = tqdm(range(num_training_steps), desc=des)
    for epoch in range(num_epochs):
        diff_out_path = "../output/{}/{}/{}/".format(model, project,train_percentage)
        # train
        prompt_model.train()
        tot_loss = 0
